export function use_find_buddy() {
    ...
    function find_buddy(list, target_id) {
        ...
        let target_city_index = list.indexOf(target_entry)
        ...
        if (target_city_index == 0) {
            ...
            let bigger_neighbor_index = target_city_index + 1
            let smaller_neighbor_index = target_city_index - 1

            let bigger_neighbor_entry = list[bigger_neighbor_index]
            let smaller_neighbor_entry = list[smaller_neighbor_index]

            let target_city_pop = parseInt(target_entry.population)
            let bigger_neighbor_pop = parseInt(bigger_neighbor_entry.population)
            let smaller_neighbor_pop = parseInt(smaller_neighbor_entry.population)

            let bigger_neighbor_diff = bigger_neighbor_pop - target_city_pop
            let smaller_neighbor_diff = target_city_pop - smaller_neighbor_pop
